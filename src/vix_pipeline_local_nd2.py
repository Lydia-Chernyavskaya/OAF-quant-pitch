#!/usr/bin/env python3
"""
VIX Analysis (N(d2) bucket variant).

Identical to vix_pipeline_local.py except that get_strikes_in_delta_bucket
and _signed_delta classify strikes by N(d2) (risk-neutral probability of
finishing in the money) instead of N(d1) (BS delta). All other logic —
F1/F2 scalar arithmetic, F3-F6 hybrid swap construction, IV solver,
variance integral, skew construction, two-expiry interpolation — is
unchanged. This file is for A/B comparison against the N(d1) version,
not a replacement.

Bucket boundaries (±0.15, ±0.45) retain their numerical values; what
they threshold is the only change. N(d2) is uniformly smaller than N(d1)
by sigma·sqrt(T)·N'(d1) at first order, so strike coverage of each
bucket will shift toward more OTM strikes.
"""
...
def main():
    ...
    if residual_rows:
        ...
        print("\n" + "=" * 70)
        print(f"HYBRID SUMMARY (N(d2)) (smoothing_sigma={HYBRID_SMOOTHING_SIGMA})")
        print("=" * 70)
    ...
    hybrid_csv_path = os.path.join(output_dir_hybrid, "vix_decomposition_hybrid_nd2.csv")
    hybrid_rows = []
    ...
    decomp_csv_path = os.path.join(output_dir, "vix_decomposition_local_nd2.csv")
    decomp_rows = []
    for i, res in enumerate(results):
        decomp = decompositions[i] if i > 0 else None
        row = {
    ...
    # ── Save to file ───────────────────────────────────────────────────────
    output_path = os.path.join(os.path.dirname(__file__), "vix_results_nd2.txt")
    ...
    try:
        ...
        plot_path = os.path.join(output_dir, "vix_decomposition_local_nd2_chart.png")

#!/usr/bin/env python3
"""
VIX Analysis — CBOE single-strike representative method, N(d2) variant.

Replicates the worked examples in the CBOE Aug 2025 whitepaper (Yen-Carry
Unwind walkthrough on pages 13, 18, 20, 22). For each of F3-F6, picks ONE
representative strike on t1's chain at the canonical target delta
(0.30 for shoulders, 0.10 for wings) and uses that strike's t1 vs t0 IV
difference (minus a control term for orthogonality) as the single-point
input to a VIX recomputation. F1/F2 use strike-aligned IV reads at the
model-free ATM strike K_atmf (the strike where |cmid − pmid| is minimized,
which is the same K0 anchor used by the VIX² formula's forward
adjustment).

KNOWN CAVEATS
- This is the SINGLE-STRIKE REPRESENTATIVE method matching CBOE's worked
  examples, not the bucket-swap method described in CBOE's prose
  ("re-computation of the VIX Index after adjusting prices for all puts
  along the 15-45 delta strike range"). The single-strike version is a
  faithful reproduction of CBOE's illustrated calculation but uses less
  information than the full bucket recomputation.
- N(d2) delta convention (vs the unspecified delta convention in the
  whitepaper) shifts the representative strike toward more-OTM levels.
- Accordion handling at the wing strike: if K_rep (t1's 10Δ) is absent
  from t0's variance integration range, we linearly interpolate
  sigma_t0(K_rep) from t0's two flanking in-range strikes. CBOE's prose
  acknowledges activation on p21 but doesn't specify the mechanic.
- F1/F2 use the model-free ATM strike (|C−P| min via put-call parity)
  rather than round(spot). This is the same anchor the VIX² formula uses
  internally for K0. CBOE's worked example on p18 uses round(spot)
  directly; on SPX the difference between the two is at most 1-2 strikes
  and well below noise floor.
"""
...
def compute_factor_single_strike(prev: dict, curr: dict,
                                 target_delta: float, side: str,
                                 control_subtract: float) -> float:
    """
    CBOE single-strike representative factor. Matches the worked examples
    in the Aug 2025 whitepaper (pages 20 and 22).

    1. K_rep = strike on t1's near chain whose N(d2)-delta is closest to
       target_delta.
    2. sigma_t1 = curr's smile IV at K_rep on the relevant side dict.
       If K_rep isn't in that dict, return 0.0.
    3. sigma_t0 = prev's smile IV at K_rep, same-side dict. If K_rep isn't
       listed (accordion case), linearly interpolate from prev's two
       flanking strikes in that dict. If neighbors are missing on either
       side, return 0.0.
    4. excess_bid = (sigma_t1 - sigma_t0) - control_subtract.
    5. Reprice K_rep's mid in t0's near AND far chains using BS price at
       vol = sigma_t0 + excess_bid. If K_rep isn't in the chain, append a
       synthetic row with the new mid (and a sentinel mid on the other
       side).
    6. Recompute VIX from the patched chains via the existing
       compute_vix_variance + two-expiry interpolation logic. Return
       vix_hybrid - prev['vix_computed'].

    target_delta: -0.30 for F3, +0.30 for F4, -0.10 for F5, +0.10 for F6.
    side: 'put' for F3/F5, 'call' for F4/F6.
    control_subtract: F2 (vol-pt) for F3/F4. F2+F3 for F5. F2+F4 for F6.
    Mixed units (vol-pt for F2, VIX-pt for F3/F4) are CBOE's convention;
    see whitepaper page 22 walkthrough where 3.43 - 2.77 = 0.66 mixes
    them.
    """
    F_old      = prev["F"]
    F_far_old  = prev["F2"]
    ...
    same_side_old = prev.get("put_skew_30d" if side == "put" else "call_skew_30d", {})
    same_side_new = curr.get("put_skew_30d" if side == "put" else "call_skew_30d", {})
    if not same_side_old or not same_side_new:
        return 0.0
    ...
    # ── Step 2: t1 IV at K_rep, same-side dict ────────────────────────────
    if K_rep not in same_side_new:
        return 0.0
    sigma_t1 = float(same_side_new[K_rep])

    # ── Step 3: t0 IV at K_rep, with accordion fallback ───────────────────
    sigma_t0 = _interp_iv_at_strike(same_side_old, K_rep)
    if sigma_t0 is None:
        return 0.0

    # ── Step 4: excess bid (mixed-units subtraction per CBOE) ────────────
    excess_bid = (sigma_t1 - sigma_t0) - control_subtract
    sigma_eff = sigma_t0 + excess_bid
    if sigma_eff <= 0 or not np.isfinite(sigma_eff):
        return 0.0
    sigma_dec = sigma_eff / 100.0

    # ── Step 5: reprice K_rep's mid in t0's chains ───────────────────────
    bs_price = _bs_put if side == "put" else _bs_call
    mid_col  = "pmid" if side == "put" else "cmid"
    other_mid_col = "cmid" if side == "put" else "pmid"

    def _patched(df_orig: pd.DataFrame, F: float, T: float) -> pd.DataFrame:
        df = df_orig.copy()
        new_price = bs_price(F, K_rep, T, sigma_dec, r)
        if not np.isfinite(new_price) or new_price <= 0:
            return df
        if (df["strike"] == K_rep).any():
            df.loc[df["strike"] == K_rep, mid_col] = new_price
            return df
        # Synthetic row: K_rep not in chain. Append with the new mid and
        # sentinels on the other-side mid + bids/asks (need both bids
        # positive for the per-side bid filter to admit the row in the
        # variance integral).
        synth = {
            "strike": float(K_rep),
            "cmid": float("nan"),
        ...
        synth[mid_col] = float(new_price)
        # Set this side's bid/ask to a positive sentinel around the mid so
        # zero-bid truncation doesn't reject it.
        if side == "put":
            synth["pbid"] = max(new_price * 0.95, 0.05)
            synth["pask"] = new_price * 1.05
        else:
            synth["cbid"] = max(new_price * 0.95, 0.05)
            synth["cask"] = new_price * 1.05
        # The other side gets a sentinel that will fail the per-side bid
        # check (zero bid) so it's not used; that's fine for the variance
        # integral because the strike's contribution comes from the
        # relevant side based on K_rep vs K0.
        out = pd.concat([df, pd.DataFrame([synth])], ignore_index=True)
        return out.sort_values("strike").reset_index(drop=True)

    df_near_patched = _patched(prev["chain1_df"], F_old, T1)
    df_far_patched  = _patched(prev["chain2_df"], F_far_old, T2)

    # ── Step 6: recompute VIX with the patched chains ─────────────────────
    ...
    return vix_hybrid - prev["vix_computed"]


def run_decomposition(prev: dict, curr: dict) -> VIXDecomposition | None:
    """
    6-factor VIX decomposition — CBOE single-strike representative method
    (Aug 2025 whitepaper, walkthrough on pages 13, 18, 20, 22).
    ...
    F3–F6 use the single-strike representative method (whitepaper pp 20, 22
    walkthrough). For each factor, identify ONE strike on t1's chain at
    the canonical target delta, look up t1's and t0's IV at that strike,
    subtract a control term, and recompute VIX with K_rep's price patched
    to vol = sigma_t0 + excess_bid:
        F3 put shoulder   target delta = -0.30   excess = (σ_t1 − σ_t0) − F2
        F4 call shoulder  target delta = +0.30   excess = (σ_t1 − σ_t0) − F2
        F5 put wing       target delta = -0.10   excess = (σ_t1 − σ_t0) − (F2 + F3)
        F6 call wing      target delta = +0.10   excess = (σ_t1 − σ_t0) − (F2 + F4)
    ...
    Target deltas 0.30 (shoulders) and 0.10 (wings) match the whitepaper's
    worked example exactly. N(d2) is used as the delta convention.
    """
    ...
    F3 = compute_factor_single_strike(prev, curr,
                                      target_delta=-0.30, side="put",
                                      control_subtract=F2)
    F4 = compute_factor_single_strike(prev, curr,
                                      target_delta=+0.30, side="call",
                                      control_subtract=F2)
    F5 = compute_factor_single_strike(prev, curr,
                                      target_delta=-0.10, side="put",
                                      control_subtract=F2 + F3)
    F6 = compute_factor_single_strike(prev, curr,
...
def main():
    ...
    parser.add_argument(
        ...
        help="(deprecated; no effect in the CBOE single-strike pipeline). "
    ...
    if residual_rows:
        ...
        print("\n" + "=" * 70)
        print(f"CBOE SINGLE-STRIKE SUMMARY (N(d2))")
        print("=" * 70)
    ...
    hybrid_csv_path = os.path.join(output_dir_hybrid, "vix_decomposition_CBOE_hybrid_nd2.csv")
    hybrid_rows = []
    ...
    decomp_csv_path = os.path.join(output_dir, "vix_decomposition_CBOE_nd2.csv")
    decomp_rows = []
    for i, res in enumerate(results):
        decomp = decompositions[i] if i > 0 else None
        row = {
    ...
    # ── Save to file ───────────────────────────────────────────────────────
    output_path = os.path.join(os.path.dirname(__file__), "vix_results_CBOE_nd2.txt")
    ...
    try:
        ...
        plot_path = os.path.join(output_dir, "vix_decomposition_CBOE_nd2_chart.png")
